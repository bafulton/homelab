# Cloudflare OpenTofu Configuration

This OpenTofu configuration manages Cloudflare Tunnel and DNS records for routing public traffic to the homelab Kubernetes cluster.

> **Engine:** [OpenTofu](https://opentofu.org/) (`tofu`), not HashiCorp Terraform. The `.tf` files and `terraform.tfstate` filenames are unchanged — OpenTofu is a drop-in for the pre-BUSL Terraform 1.5.x these were written against. Install with `brew install opentofu`.
...
Currently, this configuration is applied manually:
...
- The tunnel secret is randomly generated by OpenTofu and stored in state
